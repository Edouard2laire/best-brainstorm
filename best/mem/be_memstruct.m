...
for ii = 1:nb_clusters
    ...
    if isfield(OPTIONS.optional.clustering, 'initial_sigma')
        ...
        if OPTIONS.model.depth_weigth_MEM > 0 
            active_var{ii} = obj.GreenM2(cID{ii},cID{ii})' *  diag(OPTIONS.automatic.Sigma_s(cID{ii})) * obj.GreenM2(cID{ii},cID{ii})  * OPTIONS.solver.active_var_mult * mean( obj.Jmne(cID{ii}).^2) ;
        else
